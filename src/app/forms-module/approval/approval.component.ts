  ...
  loadId() {
    if (this.approvalType !== undefined && this.approvalType !== null) {
      ...
      if (this.approvalType === 1) {
        this.requestId = this.login_user.userID;
        this.RequestidOptions = [{ label: this.login_user.username, value: this.login_user.userID }];
        ...
        this._restApiService.getByParameters(PathConstants.Registration_Get, params).subscribe(res => {
          if (res !== undefined && res !== null && res.length !== 0) {
            res.forEach(r => {
              studentSelection.push({ label: r.emisno, value: r.studentId });
